...
unsafe impl FlatValidate for SizedEnum {
    unsafe fn validate_unchecked(bytes: &[u8]) -> Result<(), Error> {
        ...
        match tag {
            0 => Ok(()),
            1 => unsafe { iter::BytesIter::new_unchecked(bytes, iter::type_list!(u16, u8)) }
                .validate_all()
                .map_err(|e| e.offset(data_offset)),
            2 => unsafe { iter::BytesIter::new_unchecked(bytes, iter::type_list!(u8, u16)) }
                .validate_all()
                .map_err(|e| e.offset(data_offset)),
            3 => unsafe { iter::BytesIter::new_unchecked(bytes, iter::type_list!(u32)) }
                .validate_all()
                .map_err(|e| e.offset(data_offset)),
            _ => Err(Error {
