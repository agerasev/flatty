...
pub struct SharedSender<M: Flat + ?Sized, B: AsyncWriteBuffer> {
    shared: Arc<Mutex<B>>,
    buffer: AlignedBytes,
...
#[cfg(feature = "io")]
impl<M: Flat + ?Sized, P: AsyncWrite + Unpin> SharedSender<M, IoBuffer<P>> {
...
impl<M: Flat + ?Sized, B: AsyncWriteBuffer> Clone for SharedSender<M, B> {
    fn clone(&self) -> Self {
        Self {
...
impl<'a, M: Flat + ?Sized, B: AsyncWriteBuffer + 'a> SendGuard<'a, M, B> {
    pub async fn send(self) -> Result<(), SendError<B::Error>> {
